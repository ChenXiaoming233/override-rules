...
(() => {
  ...
  // src/constants.ts
  var NODE_SUFFIX, LOW_COST_SUFFIX, AUTO_SUFFIX, CDN_URL, OTHER_ICON, PROXY_GROUPS, LOW_COST_NODE_MATCHER, LANDING_NODE_MATCHER, HIGH_COST_SUFFIX, HIGH_COST_NODE_MATCHER, countriesMeta;
  var init_constants = __esm({
    "src/constants.ts"() {
      ...
      OTHER_ICON = `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Multi_Flag.png`;
      PROXY_GROUPS = {
        SELECT: "选择代理",
        MANUAL: "手动选择",
        AUTO: "自动选择",
      ...
      HIGH_COST_SUFFIX = " 高倍率";
      HIGH_COST_NODE_MATCHER = createCaseInsensitiveNodeMatcher(
        String.raw`[2-9]\.[0-9]|高倍率|Premium|旗舰|专线|VIP`
      );
      countriesMeta = {
        香港: {
          weight: 10,
          pattern: "(?i)香港|港|\\b(?:HK|hk)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Hong Kong|HongKong|hongkong|HONG KONG|HONGKONG|深港|HKG|九龙|Kowloon|新界|沙田|荃湾|葵涌|🇭🇰",
        ...
        台湾: {
          weight: 20,
          pattern: "(?i)台|新北|彰化|\\b(?:TW|tw)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Taiwan|TAIWAN|TWN|TPE|ROC|🇹🇼|🇼🇸",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Taiwan.png`
        },
        新加坡: {
          weight: 30,
          pattern: "(?i)新加坡|坡|狮城|\\b(?:SG|sg)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Singapore|SINGAPORE|SIN|🇸🇬",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Singapore.png`
        },
        日本: {
          weight: 40,
          pattern: "(?i)日本|川日|东京|大阪|泉日|埼玉|沪日|深日|\\b(?:JP|jp)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Japan|JAPAN|JPN|NRT|HND|KIX|TYO|OSA|关西|Kansai|KANSAI|🇯🇵",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Japan.png`
        },
        韩国: {
          weight: 45,
          pattern: "(?i)韩国|韩|韓|春川|Chuncheon|首尔|\\b(?:KR|kr)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Korea|KOREA|KOR|ICN|🇰🇷",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Korea.png`
        },
        美国: {
          weight: 50,
          pattern: "(?i)美国|美|波特兰|达拉斯|俄勒冈|凤凰城|费利蒙|硅谷|拉斯维加斯|洛杉矶|圣何塞|圣克拉拉|西雅图|芝加哥|纽约|亚特兰大|迈阿密|华盛顿|\\b(?:US|us)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|United States|UnitedStates|UNITED STATES|USA|America|AMERICA|JFK|EWR|IAD|ATL|ORD|MIA|NYC|LAX|SFO|SEA|DFW|SJC|🇺🇸",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/United_States.png`
        },
        加拿大: {
          weight: 55,
          pattern: "(?i)加拿大|渥太华|温哥华|卡尔加里|蒙特利尔|Montreal|\\b(?:CA|ca)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Canada|CANADA|CAN|YVR|YYZ|YUL|🇨🇦",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Canada.png`
        },
        英国: {
          weight: 60,
          pattern: "(?i)英国|伦敦|曼彻斯特|Manchester|\\b(?:UK|uk)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Britain|United Kingdom|UNITED KINGDOM|England|GBR|LHR|MAN|🇬🇧",
        ...
        德国: {
          weight: 70,
          pattern: "(?i)德国|德|柏林|法兰克福|慕尼黑|Munich|\\b(?:DE|de)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Germany|GERMANY|DEU|MUC|🇩🇪",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Germany.png`
        },
        法国: {
          weight: 80,
          pattern: "(?i)法国|法|巴黎|马赛|Marseille|\\b(?:FR|fr)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|France|FRANCE|FRA|CDG|MRS|🇫🇷",
        ...
        阿根廷: {
          pattern: "(?i)阿根廷|布宜诺斯艾利斯|\\b(?:AR|ar)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Argentina|EZE|🇦🇷",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Argentina.png`
        },
        芬兰: {
          pattern: "(?i)芬兰|赫尔辛基|\\b(?:FI|fi)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Finland|HEL|🇫🇮",
        ...
        菲律宾: {
          pattern: "(?i)菲律宾|马尼拉|\\b(?:PH|ph)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Philippines|MNL|🇵🇭",
          icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Philippines.png`
        },
        土耳其: {
          pattern: "(?i)土耳其|伊斯坦布尔|\\b(?:TR|tr)(?:[-_ ]?\\d+(?:[-_ ]?[A-Za-z]{2,})?)?\\b|Turkey|Türkiye|IST|🇹🇷",
  ...
  function classifyCountryNodes(allNodes, lowCostSet, highCostSet) {
  ...
  function buildCountryProxyGroups({
    ...
    const groups = [];
    const lowCostSubGroups = [];
    const highCostSubGroups = [];
    const autoSubGroups = [];
    const hasAutoSplit = autoSplit && groupType === 0;
    const lowCostSet = splitLowCost || hasAutoSplit ? new Set(lowCostNodes.map((n) => n.name)) : null;
    const highCostSet = splitHighCost || hasAutoSplit ? new Set(highCostNodes.map((n) => n.name)) : null;
    const needsClassification = splitLowCost || splitHighCost || hasAutoSplit;
    const nodesByCountry = !regexFilter || needsClassification ? Object.fromEntries(
    ...
    for (const country of countries) {
      ...
      if (!needsClassification || !nodesByCountry) {
        groups.push(
          buildGroupByType({
            ...
            icon,
            groupType,
            nodeSource: !regexFilter ? { proxies: nodesByCountry?.[country] ?? [] } : {
              "include-all": true,
              filter: meta.pattern,
      ...
      const hasAuto = hasAutoSplit && regular.length > 0;
      if (!hasLowCost && !hasHighCost && !hasAuto) {
        groups.push(
          buildGroupByType({
            name: `${country}${NODE_SUFFIX}`,
            icon,
            groupType,
            nodeSource: !regexFilter ? { proxies: regular.map((n) => n.name).filter(isNotNull) } : {
              "include-all": true,
              filter: meta.pattern,
              ...landing ? { "exclude-filter": LANDING_NODE_MATCHER.pattern } : {}
            }
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      ...countryProxyGroups,
      smallCountryGroup !== void 0 ? smallCountryGroup : null,
      {
      ...
      ...countryHighCostGroups,
      highCostNodes.length > 0 || regexFilter ? buildGroupByType({
        name: PROXY_GROUPS.HIGH_COST,
        icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Lab.png`,
        groupType,
        nodeSource: !regexFilter ? { proxies: highCostNodes.map((node) => node.name).filter(isNotNull) } : { "include-all": true, filter: HIGH_COST_NODE_MATCHER.pattern }
      }) : null,
      ...countryAutoGroups
    ];
  ...
  // src/node_parser.ts
  function parseSmallCountries(countryNodes, threshold) {
    const small = {};
    const regular = {};
  ...
  function parseHighCost(nodes, excludeNodes = []) {
  ...
  var init_rules = __esm({
    "src/rules.ts"() {
      ...
      baseRules = [
        ...
        `RULE-SET,QiufengADBlock,${PROXY_GROUPS.AD_BLOCK}`,
        ...
        `AND,((RULE-SET,StaticResources),(GEOIP,cn)),DIRECT`,
        `AND,((RULE-SET,CDNResources),(GEOSITE,cn)),DIRECT`,
        `AND,((RULE-SET,CDNResources),(GEOIP,cn)),DIRECT`,
        `AND,((RULE-SET,AdditionalCDNResources),(GEOSITE,cn)),DIRECT`,
        `AND,((RULE-SET,AdditionalCDNResources),(GEOIP,cn)),DIRECT`,
        ...
        `GEOSITE,steam,${PROXY_GROUPS.GAMING}`,
        ...
        `GEOSITE,rockstar,${PROXY_GROUPS.GAMING}`,
        ...
        `GEOSITE,bing,${PROXY_GROUPS.BING}`,
        `GEOSITE,onedrive,${PROXY_GROUPS.ONEDRIVE}`,
  ...
  var require_main = __commonJS({
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        let smallCountryNodes = [];
        let smallCountryPatterns = [];
        let filteredCountryNodes = countryNodes;
        if (liteCombine > 0) {
          const { small, regular } = parseSmallCountries(countryNodes, liteCombine);
          smallCountryNodes = Object.values(small).flat().map((n) => n.name).filter(Boolean);
          smallCountryPatterns = Object.keys(small).map((c) => countriesMeta[c]?.pattern).filter(Boolean);
          filteredCountryNodes = regular;
        }
        const countryNames = getActiveCountryNames(filteredCountryNodes, countryThreshold);
        let smallCountryGroup = null;
        const smallCountryGroupName = smallCountryNodes.length > 0 ? PROXY_GROUPS.OTHER : void 0;
        if (smallCountryGroupName) {
          const smallCountryNodeSource = !regexFilter ? { proxies: smallCountryNodes } : { "include-all": true, filter: "(?i)" + smallCountryPatterns.join("|") };
