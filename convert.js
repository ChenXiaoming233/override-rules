...
(() => {
      ...
      if (!hasLowCost && !hasHighCost && !hasAuto) {
        groups.push(
          buildGroupByType({
            ...
            icon,
            groupType,
            nodeSource: !regexFilter ? { proxies: regular } : {
              "include-all": true,
              filter: meta.pattern,
              ...landing ? { "exclude-filter": LANDING_NODE_MATCHER.pattern } : {}
            }
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      highCostNodes.length > 0 || regexFilter ? buildGroupByType({
        ...
        icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Lab.png`,
        groupType,
        nodeSource: !regexFilter ? { proxies: highCostNodes } : { "include-all": true, filter: HIGH_COST_NODE_MATCHER.pattern }
      }) : null,
      ...countryAutoGroups
    ];
  ...
  // src/node_parser.ts
  function parseSmallCountries(countryInfo, threshold) {
    const small = [];
    const regular = [];
  ...
  function parseHighCost(config, excludeNodes = []) {
    ...
    }).map((proxy) => proxy.name).filter((name) => Boolean(name));
  }
  ...
  var init_rules = __esm({
    "src/rules.ts"() {
      ...
      baseRules = [
        ...
        `AND,(RULE-SET,StaticResources),(GEOIP,cn),DIRECT,no-resolve`,
        `AND,(RULE-SET,CDNResources),(GEOSITE,cn),DIRECT`,
        `AND,(RULE-SET,CDNResources),(GEOIP,cn),DIRECT,no-resolve`,
        `AND,(RULE-SET,AdditionalCDNResources),(GEOSITE,cn),DIRECT`,
        `AND,(RULE-SET,AdditionalCDNResources),(GEOIP,cn),DIRECT,no-resolve`,
    ...
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        if (liteCombine > 0) {
          ...
          smallCountryPatterns = small.map((item) => countriesMeta[item.country]?.pattern).filter(Boolean);
        ...
        const allCountryGroupNames = smallCountryGroupName ? [...countryGroupNames, smallCountryGroupName] : countryGroupNames;
        const {
